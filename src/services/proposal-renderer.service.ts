...
const ICON_SVG: Record<IconName, string> = {
  ...
  calendar: '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>',
  globe:    '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>',
  ...
  zap:      '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>',
  ...
  mapPin:   '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>',
};
...
<div class="card rounded-xl p-5">
  <div class="text-lg" aria-hidden="true">${ICON_SVG[s.icon]}</div>
  <h3 class="mt-3 font-medium">${escapeHtml(s.title)}</h3>
  <p class="mt-1 text-sm muted">${escapeHtml(s.description)}</p>
</div>`)
    .join('\n');
...
function renderTestimonials(content: SiteContent): string {
  ...
  const items = content.testimonials
    .map((t) => `
<blockquote class="card rounded-xl p-5">
  <p class="text-base italic">&ldquo;${escapeHtml(t.quote)}&rdquo;</p>
  <footer class="mt-2 text-sm muted">— ${escapeHtml(t.attribution)}</footer>
</blockquote>`)
    .join('\n');
...
<div class="card rounded-xl p-4">
  <dt class="text-xs uppercase muted tracking-wide">${escapeHtml(label)}</dt>
  <dd class="mt-1 font-medium">${escapeHtml(value)}</dd>
</div>`)
    .join('\n');
...
function renderDiagnosis(content: SiteContent): string {
  const bullets = content.diagnosis.bullets
    .map((b) => `
<li class="card flex gap-3 rounded-xl p-4">
  <span class="mt-0.5 text-lg" aria-hidden="true">${ICON_SVG[b.icon]}</span>
  <div>
    <p class="font-medium">${escapeHtml(b.label)}</p>
    <p class="text-sm muted mt-1">${escapeHtml(b.evidence)}</p>
  </div>
</li>`)
    .join('\n');
...
function renderPricing(
  ...
  const valueBullets = content.pricingPitch.valueBullets
    .map((b) => `<li class="flex gap-2"><span aria-hidden="true">✓</span><span>${escapeHtml(b)}</span></li>`)
    .join('\n');
...
function renderCtaForm(
  ...
  const tier = proposal.finalTier ?? 'Starter';
  const cents = proposal.priceCents ?? priceForTier(tier);
  const paymentLinkButton = proposal.paymentLinkUrl
    ? `<a href="${escapeHtml(proposal.paymentLinkUrl)}" target="_blank" rel="noopener" class="block text-center rounded-lg border px-4 py-3 font-medium">Pay ${formatUsd(cents)} via secure checkout</a>`
    : '';
